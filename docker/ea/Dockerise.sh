...
docker rmi frontend:ea middleware:ea

# Switch to Master branch in case we're not in the right branch
# TODO UPDATE THIS TO master-ea branch when it goes to public repo
...
# Set the Time
export TZ=NZ
export BUILD_DATE_EA=$(date +%Y-%m-%d)-$(date +%T)

# Get the latest git commit's hash
export GIT_SHA_EA=`git rev-parse --short HEAD`
